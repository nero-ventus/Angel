 ...
 * @author devil
 */
@WebServlet(name = "actualiza_configuracion_le", urlPatterns = {"/actualiza_configuracion_le"})
public class actualiza_configuracion_le extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet actualiza_configuracion_le</title>");            
            out.println("<script>\n" +
"            function presiona1(){\n" +
"                setTimeout(function (){\n" +
"                    window.location.replace('simula_externa.jsp');\n" +
"                },0);\n" +
"            }\n" +
"            function presiona2(){\n" +
"                setTimeout(function (){\n" +
...
"        </script>");
            ...
            if (valida1 == 1) {
                ...
                if (!(usua == null)) {
                    ...
                    config_lexterna config_le=null;
                    try{
                        int volumen=Integer.parseInt(request.getParameter("volumen")),estado_servicio=0;
                        int distap=Integer.parseInt(request.getParameter("distap"));
                        String tono=request.getParameter("tono");
                        try{
                            estado_servicio=Integer.parseInt(request.getParameter("estado_servicio"));
                        }
                        catch(Exception ee){
                            System.out.println(ee.getMessage()+"No importa");
                            estado_servicio=2;
                        }
                        if(-1<volumen && volumen<11 && -1<distap && distap<21){
                            noti=new notificacion(0, volumen, tono, 0);
                            config_le=new config_lexterna(0, distap, estado_servicio, 0, 0);
                            Iconfig_lexternadao controla_le=new impconfig_lexterna();
                            int validac=controla_le.actualizaconfig_lexterna(config_le, correo);
                            if(validac==1){
