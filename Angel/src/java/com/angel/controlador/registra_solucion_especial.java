 ...
 */
@WebServlet(name = "registra_solucion_especial", urlPatterns = {"/registra_solucion_especial"})
public class registra_solucion_especial extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Solucionando...</title>");
            out.println("<script>\n"
                    + "            function presiona1(){\n"
                    + "                setTimeout(function (){\n"
                    + "                    window.location.replace('consulta_reportes_mantenimiento_gerente.jsp');\n"
                    + "                },0);\n"
                    + "            }\n"
                    + "            function presiona2(){\n"
                    + "                setTimeout(function (){\n"
            ...
            out.println("</head>");
            out.println("<body>");
            Ireporte_mantenimientodao controla_gerente = new impreporte_mantenimiento();
            String solucion = request.getParameter("solucion");
            int id_repor = Integer.parseInt(request.getParameter("id_repor"));
            int valida = controla_gerente.actualizareporte_mantenimiento_especial(id_repor, solucion);
            if (valida == 1) {
