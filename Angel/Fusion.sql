...
-------------------------------------------------------------
-------------------------------------------------------------
create table gerente_mantenimiento(
	correo varchar(100) primary key,
    contra varchar(30) not null,
    nombre varchar(30),
    apellidos varchar(30)
...
create table reporte_mantenimiento(
	id_repor_mantenimiento int primary key auto_increment,
    fecha date,
    estado int,
    descripcion varchar(300),
    solucion varchar(300),
    tipo varchar(10),
    correo_gere varchar(100)
);
/*
	estado:
    1=Abierto
...
*/
-------------------------------------------------------------
create table etiquetas_mantenimiento(
	id_etiqueta int primary key auto_increment,
    nombre varchar(30)
);
-------------------------------------------------------------
create table rela_etiquetas_reporte_mantenimiento(
	id_rela_eti_repor int primary key auto_increment,
    id_reporte_mantenimiento int,
    id_etiqueta int,
    cantidad int,
    foreign key(id_reporte_mantenimiento) references angel.reporte_mantenimiento (id_repor_mantenimiento),
    foreign key(id_etiqueta) references angel.etiquetas_mantenimiento (id_etiqueta)
);
-------------------------------------------------------------
-------------------------------------------------------------
create table rela_reporte_evento_mantenimiento(
	id_rela_re_even_mante int primary key auto_increment,
    id_repor_mantenimiento int,
    id_repor_evento int,
    estado int,
    foreign key (id_repor_mantenimiento) references reporte_mantenimiento (id_repor_mantenimiento),
    foreign key (id_repor_evento) references reporte_evento (id_repor_evento)
);
/*
	estado
    1=En proceso
...
delimiter ;
/*---------------------------------------------------------------------------*/
drop procedure if exists sp_getdispositivos_ocupados;
delimiter **
create procedure sp_getdispositivos_ocupados(in correou varchar(100))
begin
declare codigoc varchar(16);
set codigoc=(select angel.codigo_venta.codigo from angel.codigo_venta inner join angel.cuenta_usuario where binary angel.codigo_venta.codigo=angel.cuenta_usuario.codigo and binary angel.cuenta_usuario.correo=correou);
select angel.dispositivo.* from angel.dispositivo where exists(select angel.rela_dispo_infante.* from angel.rela_dispo_infante where binary angel.rela_dispo_infante.id_dispositivo=angel.dispositivo.id_dispositivo) and binary angel.dispositivo.codigo=codigoc;
end;**
delimiter ;
/*---------------------------------------------------------------------------*/
...
delimiter ;
/*---------------------------------------------------------------------------*/
drop procedure if exists sp_registra_reporte_evento_ingeniero;
delimiter **
create procedure sp_registra_reporte_evento_ingeniero(in fechau date,in descripcionu varchar(300),in correo_usuau varchar(100),in correo_ingeu varchar(100))
begin
insert into angel.reporte_evento values(0,fechau,1,descripcionu,'Sin Resolver',correo_ingeu,correo_usuau);
select 1;
end;**
delimiter ;
/*---------------------------------------------------------------------------*/
...
begin
select angel.reporte_evento.* from angel.reporte_evento where binary angel.reporte_evento.estado=2;
end;**
delimiter ;
/*---------------------------------------------------------------------------*/
...
delete angel.rela_reporte_evento_mantenimiento from angel.rela_reporte_evento_mantenimiento where binary angel.rela_reporte_evento_mantenimiento.id_repor_evento=id_reporteu;
...
/*---------------------------------------------------------------------------*/
/*---------------------------------------------------------------------------*/
drop procedure if exists sp_registra_gerente_mantenimiento;
delimiter **
create procedure sp_registra_gerente_mantenimiento(in correou varchar(100),in contrau varchar(30),in nombreu varchar(30),in apellidosu varchar(30))
begin
insert into angel.gerente_mantenimiento values(correou,contrau,nombreu,apellidosu);
select 1;
end;**
delimiter ;
/*---------------------------------------------------------------------------*/
drop procedure if exists sp_trae_gerente_mantenimiento;
delimiter **
create procedure sp_trae_gerente_mantenimiento(in correou varchar(100),in contrau varchar(30))
begin
select angel.gerente_mantenimiento.* from angel.gerente_mantenimiento where binary angel.gerente_mantenimiento.correo=correou and binary angel.gerente_mantenimiento.contra=contrau;
end;**
delimiter ;
/*---------------------------------------------------------------------------*/
drop procedure if exists sp_registra_programador;
delimiter **
create procedure sp_registra_programador(in correou varchar(100),in contrau varchar(30),in nombreu varchar(30),in apellidosu varchar(30))
begin
insert into angel.programador values(correou,contrau,nombreu,apellidosu);
select 1;
end;**
delimiter ;
/*---------------------------------------------------------------------------*/
drop procedure if exists sp_trae_programador;
delimiter **
create procedure sp_trae_programador(in correou varchar(100),in contrau varchar(30))
begin
select angel.programador.* from angel.programador where binary angel.programador.correo=correou and binary angel.programador.contra=contrau;
end;**
delimiter ;
/*---------------------------------------------------------------------------*/
drop procedure if exists sp_registra_reporte_mantenimiento;
delimiter **
create procedure sp_registra_reporte_mantenimiento(in fechau date,in descripcionu varchar(300),in tipou varchar(10), in correo_greu varchar(100))
begin
insert into angel.reporte_mantenimiento values(0,fechau,1,descripcionu,'Sin Resolver',tipou,correo_greu);
select 1;
end;**
delimiter ;
/*---------------------------------------------------------------------------*/
...
delimiter ;
/*---------------------------------------------------------------------------*/
drop procedure if exists sp_cierra_reporte_mantenimiento;
delimiter **
create procedure sp_cierra_reporte_mantenimiento(in id_reporteu int,in solucionu varchar(300))
begin
update angel.reporte_mantenimiento set angel.reporte_mantenimiento.estado=2,angel.reporte_mantenimiento.solucion=solucionu where binary angel.reporte_mantenimiento.id_repor_mantenimiento=id_reporteu;
select 1;
end;**
delimiter ;
/*---------------------------------------------------------------------------*/
drop procedure if exists sp_trae_reportes_mantenimiento_cerrados_solucionados;
delimiter **
create procedure sp_trae_reportes_mantenimiento_cerrados_solucionados()
begin
select angel.reporte_mantenimiento.* from angel.reporte_mantenimiento where binary angel.reporte_mantenimiento.estado=2;
end;**
delimiter ;
/*---------------------------------------------------------------------------*/
drop procedure if exists sp_trae_programadores;
delimiter **
create procedure sp_trae_programadores()
begin
select angel.programador.* from angel.programador;
end;**
delimiter ;
select * from reporte_mantenimiento;

/*---------------------------------------------------------------------------*/
drop procedure if exists sp_trae_reportes_mantenimiento_abiertos_sin_asignar;
delimiter **
create procedure sp_trae_reportes_mantenimiento_abiertos_sin_asignar()
begin
select angel.reporte_mantenimiento.* from angel.reporte_mantenimiento where binary angel.reporte_mantenimiento.estado=3 and binary angel.reporte_mantenimiento.correo_gere='Sin asignar';
end;**
delimiter ;
drop procedure if exists sp_asigna_programador;
delimiter **
create procedure sp_asigna_programador(in id_reporteu int,in correo_programador varchar(300))
begin
update angel.reporte_mantenimiento set angel.reporte_mantenimiento.correo_gere=correo_programador where binary angel.reporte_mantenimiento.id_repor_mantenimiento=id_reporteu;
select 1;
end;**
delimiter ;
/*---------------------------------------------------------------------------*/
...
/*---------------------------------------------------------------------------*/
/*---------------------------------------------------------------------------*/
drop procedure if exists sp_trae_reporte_mantenimiento_unico;
delimiter **
create procedure sp_trae_reporte_mantenimiento_unico(in correou varchar(100),in descripu varchar(300),in fechau date)
begin
select angel.reporte_mantenimiento.* from angel.reporte_mantenimiento where binary angel.reporte_mantenimiento.correo_gere=correou and binary angel.reporte_mantenimiento.descripcion=descripu and binary angel.reporte_mantenimiento.fecha=fechau;
end;**
delimiter ;
/*---------------------------------------------------------------------------*/
drop procedure if exists sp_soluciona_reporte_evento;
delimiter **
create procedure sp_soluciona_reporte_evento(in id_reporu int,in soluu varchar(300))
begin
update angel.reporte_evento set angel.reporte_evento.solucion=soluu where binary angel.reporte_evento.id_repor_evento=id_reporu;
select 1;
end;**
delimiter ;
/*---------------------------------------------------------------------------*/
drop procedure if exists sp_registra_reporte_evento_to_mantenimiento;
delimiter **
create procedure sp_registra_reporte_evento_to_mantenimiento(in fechau date,in descripcionu varchar(300),in tipou varchar(10),in id_repor_evenu int)
begin
insert into angel.reporte_mantenimiento values(0,fechau,3,descripcionu,'Sin Resolver',tipou,'Sin asignar');
insert into angel.rela_reporte_evento_mantenimiento values(0,last_insert_id(),id_repor_evenu,1);
select 1;
end;**
delimiter ;
/*---------------------------------------------------------------------------*/
drop procedure if exists sp_verifica_reporte_evento_existe_mantenimiento;
delimiter **
create procedure sp_verifica_reporte_evento_existe_mantenimiento(in id_repor_evenu int)
begin
declare cantidad int;
set cantidad=(select count(angel.rela_reporte_evento_mantenimiento.id_rela_re_even_mante) from angel.rela_reporte_evento_mantenimiento where binary angel.rela_reporte_evento_mantenimiento.id_repor_evento=id_repor_evenu);
if(cantidad>0) then
	select 1;
else
	select 2;
...
delimiter ;
/*---------------------------------------------------------------------------*/
drop procedure if exists sp_cerrar_reporte_mantenimiento_de_evento;
delimiter **
create procedure sp_cerrar_reporte_mantenimiento_de_evento(in id_repor_evenu int)
begin
declare id_manteni int;
set id_manteni=(select angel.rela_reporte_evento_mantenimiento.id_repor_mantenimiento from angel.rela_reporte_evento_mantenimiento where binary angel.rela_reporte_evento_mantenimiento.id_repor_evento=id_repor_evenu);
update angel.reporte_mantenimiento set angel.reporte_mantenimiento.estado=2 where binary angel.reporte_mantenimiento.id_repor_mantenimiento=id_manteni;
update angel.rela_reporte_evento_mantenimiento set angel.rela_reporte_evento_mantenimiento.estado=2 where binary angel.rela_reporte_evento_mantenimiento.id_repor_evento=id_repor_evenu;
select 1;
end;**
delimiter ;
/*---------------------------------------------------------------------------*/
drop procedure if exists sp_valida_cerrado_reporte_mantenimiento_de_evento;
delimiter **
create procedure sp_valida_cerrado_reporte_mantenimiento_de_evento(in id_repor_evenu int)
begin
declare verificador int;
set verificador=(select count(angel.rela_reporte_evento_mantenimiento.id_rela_re_even_mante) from angel.rela_reporte_evento_mantenimiento where binary angel.rela_reporte_evento_mantenimiento.id_repor_evento=id_repor_evenu and binary angel.rela_reporte_evento_mantenimiento.estado=2);
